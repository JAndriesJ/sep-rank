...
idx2var(var,index)        = isempty(index) ?  0.0 : map(α -> var[α],index)
idx2var(var,index,coef)   = isempty(coef) ? 0.0 : sum(coef .* idx2var(var, index))
idx2var(var,index,coef,g) = idx2var(var,g .+ index,coef)

mgf(arr) = reshape([arr...],size(arr)...)
idx2var_a(var,index,coef) = mgf(map((x,y)-> Utils_cons.idx2var(var,x,y),index,coef))
idx2var_a(var,index,coef,g) = mgf(map((x,y)-> Utils_cons.idx2var(var,x,y,g),index,coef))

idx2var(var,index::Array{Array{T,1} where T,2},coef) = mgf(map((x,y)-> Utils_cons.idx2var(var,x,y),index,coef))

function idx2var_dic(var,index,coef)
    kl = [keys(coef)...]
    f(b) = mgf(idx2var_a(var, index[b],coef[b]))
...
# """ ?"""
# function get_Gᴿ(xx̄ᵀ⨂yȳᵀ)
#     Rxx̄ᵀ_tens_yȳᵀ = xx̄ᵀ⨂yȳᵀ["real"]
#     Ixx̄ᵀ_tens_yȳᵀ = xx̄ᵀ⨂yȳᵀ["imag"]
